...
### Вариант 1: Быстрая установка через Homebrew Cask (Рекомендуется)
Если вы используете пакетный менеджер Homebrew, вы можете установить Pano одной командой в терминале:
```bash
brew install --cask ponano/pano/pano
```
Homebrew автоматически подключит репозиторий, загрузит последний релиз и установит `Pano.app` прямо в папку «Программы».

### Вариант 2: Ручная установка через образ диска DMG
Если вы предпочитаете стандартный графический установщик macOS:
1. Откройте страницу [Релизов Pano](https://github.com/ponano/androidtvremotemacos/releases) на GitHub.
2. Скачайте файл последней версии `Pano.dmg`.
3. Откройте скачанный файл `.dmg` и перетащите иконку **Pano** в папку **«Программы» (Applications)**.

### Вариант 3: Установка из исходного кода (Для разработчиков)
Если вы хотите собрать и запустить Pano вручную:
1. **Требования**: Убедитесь, что у вас установлены **macOS 12.0+**, **Node.js (v16+)** и **компилятор Swift** (входит в состав Xcode Command Line Tools).
2. **Скачайте или клонируйте** этот репозиторий.
3. **Настройка IP**: Откройте файл `run_kvm.sh` в текстовом редакторе и укажите локальный IP-адрес вашего телевизора:
   ```bash
   TV_IP="192.168.1.100"  # Укажите IP вашего Android TV
   ```
4. **Запуск**: Запустите KVM-мост через Терминал:
   ```bash
   bash run_kvm.sh
   ```
...
### 🔑 Безопасное сопряжение (Только при первом запуске)
При первом запуске Pano (любым из способов выше):
1. На экране вашего Mac появится всплывающее окно с запросом 6-значного PIN-кода.
2. Введите 6-значный PIN-код, отобразившийся на экране вашего Android TV / Google TV.
3. После этого шифрованные TLS-сертификаты будут надежно сохранены в папке `~/.tv_kvm_credentials/` (или `~/.credentials/` в режиме тестирования), и повторное сопряжение больше не потребуется.
4. **Начало работы**: Подведите курсор к выбранному краю экрана вашего Mac, задержите его на мгновение (800 мс) и начинайте управлять вашим телевизором!
...
* **Свайп одним пальцем (Вверх / Вниз / Влево / Вправо)**: Транслируется в стандартные нажатия D-pad для перемещения по плиткам и спискам (свайп влево теперь безопасно навигирует на ТВ без риска ложного выхода).
* **Скролл двумя пальцами (Вверх / Вниз)**: Управление громкостью телевизора (Громче / Тише).
* **Свайп тремя пальцами влево**: Мгновенный выход из режима управления KVM обратно на MacBook.
* **Тап тремя пальцами**: Возврат на домашний экран телевизора (Home).
